...
<small>source - https://commons.wikimedia.org</small>
...
Let's look at data sample.
```{r, Show data sample, message=F, warning=F}
## Data Sample
datatable(head(noise), options = list(searching = F, pageLength = 6))
```

We need to convert values on date and information on location(Latitude and Longitude)
...
## Set Longitude and Latitude of each borough to aviod OVER_QURY_LIMIT error when use 'get_googlemap' function
...
Noise pollution is visualized by using 'stat_density2d' function provided from ggplot2 package. This 'stat_density2d' function is to do 2D kernel density estimation. Each created ggmap for noise pollution by borough is stored in order to display properly in Result section. 
...
To create calander heat map, we need to load raw dataset and modify data.
...
The following maps are showing where noise pollution occurs and a spatial pattern of noise pollution within each borough.
...
Below plot is illustrating distribution of noise pollution by type. We can find the source of noise pollution in New York City.
...
The following calender map illustrates noise pollution by day in 2016. It is easy to make a calendar map with 'calenderPlot' function.
