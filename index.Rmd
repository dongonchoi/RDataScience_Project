...
Noise pollution is the one of serious problems especially in metropolitan areas such as New York City. Noise has a negative impact on dwellers' health or well-being lives, and also the growing body of scientific evidence is showing direct health effect (Dominici, 2013, and WHO, 2011). Thus, noise needs to be considered the one of serious problems we have been going through. In this project, I will investigate noise pollution problems in New York City by visualizing and analyzing NYC Open Data's dataset on 311 Service Requests.

# Materials and methods

## Data source: NYC311 Service Requests

NYC311 provides 24/7 support to New Yorkers by providing a one-stop shop for a multitude of general needs. Within the proximity of the New York Metropolitan Area, callers can dial 311 to request for a variety of governmental services, such as filing a noise complaint.

The data set used in this project is NYC311 Service Requests data in 2016. Each observation contains 53 variables describing the service request, such as complaint type, date received, incident address, and resolution description. To reduce computational overhead, only several columns will be used. I will visualize and analyze the data to answer the following three basic questions:
...
The dataset used in this project is available at NYC Opean data website. We can download this data from [this link](https://nycopendata.socrata.com/Social-Services/311-Service-Requests-from-2010-to-Present/erm2-nwe9). Due to the size of this dataset, I will load directly from my own computer after downloading the dataset. After loading the dataset, we need to filter out some columns to reduce computational overhead.
...
Let's look at the data sample. After filtering, the data has five columns which contain noise information on date, type, region and location(lon, lat) respectively.
```{r, Show data sample, message=F, warning=F}
## Data Sample
datatable(head(noise), options = list(searching = T, pageLength = 5))
```

We need to convert date format to 'POSIXlt'to handle date information properly. Also location values(Latitude and Longitude) need to be rounded to 3 decimals to conduct density calculation based on location. 
...
The following tasks need to be done for mapping noise pollution by borough. This part includes downloading base maps of each borough, dividing raw data by borough.
...
Noise pollution is visualized with 'stat_density2d' function provided from ggplot2 package. This 'stat_density2d' function is to do 2D kernel density estimation. Each created ggmap for noise pollution by borough is stored in order to display properly in Result section. 
...
To plot the distribution of noise pollution by type, we need to make new data from raw data.
...
To create a calander heat map, we need to load raw dataset and modify data.
...
The following maps show the spatial distribution of noise pollution by each borough. New York City has five boroughs, which are Manhattan, Brooklyn, Queens, Bronx, and Staten Island.
...
Below plot illustrates the distribution of noise pollution by type. We can find the source of noise pollution in New York City.
...
The following calender map illustrates noise pollution by each day in 2016. It is easy to make a calendar map with 'calenderPlot' function.
...
In this project, noise pollution in New York City is investigated based on three basic questions - Where, What and When. For Manhattan and Queens, noise pollution affects almost the whole area of these two boroughs. On the other hand, for Brookyn, Bronx, and Staten Island, noise pollutions are located at some areas of these boroughs. The most contributable factor to noise pollution in New York City is residential noise (about 53%). When we looked at calander heat map, noise pollution primarily occurs on weekend. Perhaps, it is because many people invite friends to their houses and have a party with playing music, making some noise. We've looked at some visualizations on noise pollution in New York City. It would be helpful to understand noise pollution in New York City.
