...
## Data Sample
datatable(head(noise), options = list(searching = F, pageLength = 3))
```
...
Noise pollution is visualized by using 2D kernel density estimation provided from ggplot2 package. Each created ggmap for noise pollution by borough is stored in order to display properly in Result section. 
...
```{r, Map noise pollution, warning=F, message=F}
## Map noise pollution by borough
#grid.arrange(mp1, mp2, mp3, mp4, mp5, ncol = 2)
```
```{r, Map noise pollution 2, warning=F, message=F, echo=F}
...
The following calander map illustrates noise pollution by day in 2016.
